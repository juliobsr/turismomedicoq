...
type EmailBrandSettings = Pick<
  EmailDeliverySettings,
  'brandName' | 'primaryColor' | 'secondaryColor' | 'accentColor' | 'backgroundColor' | 'textColor'
>
...
const fallbackBrand: EmailBrandSettings = {
  brandName: 'Elite Medical Journey',
  primaryColor: '#0EA5A3',
...
const resolveBrand = (brand?: EmailBrandSettings): EmailBrandSettings => ({
  ...fallbackBrand,
  ...brand,
})
...
const getBrandInitials = (brandName: string) => {
  ...
  return (words[0] || 'EM').slice(0, 2).toUpperCase()
}

const button = (href: string, label: string, brand: EmailBrandSettings, variant: 'primary' | 'reply' = 'primary') => {
  const background = variant === 'reply' ? brand.accentColor : brand.primaryColor
...
const replyNotice = (replyUrl: string, brand: EmailBrandSettings) => `
  <div style="margin-top: 28px; padding: 24px; border-radius: 20px; background: #ffffff; border: 2px solid ${brand.primaryColor};">
    <p style="margin: 0 0 10px; color: ${brand.textColor}; font-size: 16px; font-weight: 900;">Please use the secure reply link</p>
    ...
    ${button(replyUrl, 'Reply securely to my coordinator', brand, 'reply')}
    <p style="margin: 18px 0 0; color: #475569; font-size: 12px; line-height: 1.55;">
      Secure reply link:<br />
      <a href="${replyUrl}" style="color: ${brand.primaryColor}; word-break: break-all;">${replyUrl}</a>
    </p>
  </div>
`

const caseCard = (caseFolio: string, brand: EmailBrandSettings) => `
  <div style="margin-top: 22px; padding: 14px 16px; border-radius: 16px; background: #f8fafc; border: 1px solid #e2e8f0;">
    ...
    <p style="margin: 4px 0 0; color: ${brand.textColor}; font-size: 18px; font-weight: 900;">${escapeHtml(caseFolio)}</p>
  </div>
`

const wrapEmail = (title: string, subtitle: string, body: string, brand: EmailBrandSettings) => {
  const escapedBrandName = escapeHtml(brand.brandName)
  const initials = getBrandInitials(brand.brandName)
  const nameWords = escapedBrandName.trim().split(/\s+/)
  const firstWord = nameWords[0] || escapedBrandName
  const restOfName = nameWords.slice(1).join(' ')

  return `
  <div style="margin: 0; padding: 0; background: ${brand.backgroundColor}; font-family: Arial, Helvetica, sans-serif; color: ${brand.textColor};">
    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse: collapse; background: ${brand.backgroundColor};">
      <tr>
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="border-collapse: collapse; max-width: 640px;">
            <tr>
              <td style="padding: 0 0 16px;">
                <table role="presentation" cellspacing="0" cellpadding="0" style="border-collapse: collapse;">
                  <tr>
                    <td style="width: 48px; height: 48px; border-radius: 14px; background: ${brand.primaryColor}; color: #ffffff; font-size: 18px; font-weight: 900; text-align: center;">
                      ${initials}
                    </td>
                    <td style="padding-left: 12px;">
                      <p style="margin: 0; color: ${brand.textColor}; font-size: 18px; font-weight: 900; line-height: 1.2;">${firstWord}${restOfName ? ` <span style="color: ${brand.primaryColor};">${restOfName}</span>` : ''}</p>
            ...
            <tr>
              <td style="overflow: hidden; border-radius: 26px; background: #ffffff; border: 1px solid #dbeafe; box-shadow: 0 24px 70px rgba(15, 23, 42, 0.10);">
                <div style="padding: 34px 32px 34px; background: #ffffff; border-top: 6px solid ${brand.primaryColor}; border-bottom: 1px solid #e2e8f0;">
                  <p style="margin: 0 0 10px; color: ${brand.primaryColor}; font-size: 12px; font-weight: 900; letter-spacing: 0.12em; text-transform: uppercase;">${escapedBrandName}</p>
                  <h1 style="margin: 0; color: ${brand.secondaryColor}; font-size: 30px; line-height: 1.15; font-weight: 900;">${title}</h1>
            ...
            <tr>
              <td style="padding: 18px 6px 0; text-align: center;">
                <p style="margin: 0; color: #94a3b8; font-size: 12px; line-height: 1.6;">
                  ${escapedBrandName} helps patients coordinate medical care, travel logistics, and recovery support in Queretaro, Mexico.
                </p>
              </td>
            </tr>
          </table>
...
export const buildLeadCreationConfirmationEmail = (
  ...
  return {
    ...
    html: wrapEmail(
      ...
      `${brand.brandName} has opened your secure patient case and our coordination team is reviewing your request.`,
      `
    ...
    return {
      ...
      html: wrapEmail(
        ...
        `
          ...
          <p style="margin: 0; color: #64748b; font-size: 13px;">If the button does not work, copy this link into your browser:<br /><a href="${uploadUrl}" style="color: ${brand.primaryColor}; word-break: break-all;">${uploadUrl}</a></p>
          ${caseCard(input.caseFolio, brand)}
        `,
        brand
      ),
